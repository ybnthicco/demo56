...
<div class="card" >
<div class="header">
<h2>Здесь отображаются топы по количеству приглашенных партнеров</h2>
</div>
...
<th class="width:25%">Пригласил</th>
<th style="width:25%;">Активных</th>
<th style="width:25%;">Прибыль</th>
</tr>
</thead>
<tbody class="no-border-x">
<? 
$toprow=$db->query("SELECT id,i_have_refs_as_curator,cursum FROM `ss_users` WHERE cursum>'0' ORDER BY cursum DESC LIMIT 100");
while($top=$db->fetch($toprow)){?>
<?$wallet=$db->getOne("SELECT wallet FROM `ss_users` WHERE id=?i",$top['id']);?>
<?$activ=$db->getOne("SELECT COUNT(id) FROM `ss_users` WHERE curator=?i and psum > 0 ",$top['id']);?>
 <tr>
 <td style="width: 24%;"><?=$wallet?></td>
 <td style="width: 24%;"><?=$top['i_have_refs_as_curator']?> чел.</td>
 <td style="width: 24%;"><?=$activ?> чел.</td>
 <td style="width: 24%;"><?=$top['cursum']?> <?=$m_curr?></td>
 </tr>
<?}?>
</tbody>
</table>
